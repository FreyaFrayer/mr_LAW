...
"""panda_ik_window_viz.ik_window_compare_player

Play back Panda IK *window* experiment results (greedy vs window) in RViz2.

It reads the experiment output directory (containing summary.json, targets.json, p*.json),
reconstructs the greedy and window waypoint sequences, and publishes:

- /greedy/joint_states   (sensor_msgs/JointState)
- /window/joint_states   (sensor_msgs/JointState)
- /ik_window/markers     (visualization_msgs/MarkerArray)  # points/labels/lines

Additionally, it publishes static TF transforms to place the two robots:
  fixed_frame -> <greedy_prefix><root_link>
  fixed_frame -> <window_prefix><root_link>
...
from summary.json (e.g. timing results).
...
- Synchronized looping (sync_loop=true): both trajectories restart together each cycle.
  The cycle duration is max(T_greedy, T_window). The shorter one holds its last pose
...
def _resolve_result_dir(path: str) -> str:
    ...
    def _key(p: str) -> Tuple[int, str, float]:
        ...
        # Prefer directories that look like timestamps (YYYYMMDD_HHMMSS) or digits-only.
        is_ts = 1 if run_dir.replace("_", "").isdigit() else 0
...
def _load_targets(result_dir: str, summary: dict) -> List[dict]:
    ...
    The panda_ik_window output typically stores it as:
      {"meta":..., "p0":..., "targets": [ ... ]}
    """
    ...
    if isinstance(t, list):
        return t
    if isinstance(t, dict) and "targets" in t and isinstance(t["targets"], list):
        return t["targets"]
    return []
...
def _build_segments_from_summary(summary: dict, method: str) -> Tuple[List[Segment], List[str]]:
    """Return (segments, visit_order_names) for method in {'greedy','window'}."""
    ...
    if method == "greedy":
        ...
        win_res = summary.get("window", {})
        final_path = win_res.get("final_path", {})
...
class IKWindowComparePlayer(Node):
    def __init__(self) -> None:
        super().__init__("ik_window_compare_player")
        ...
        # In panda_ik_window outputs, tip_link is usually panda_link8.
        ...
        self.declare_parameter("window_prefix", "window_/" )

        # Base placement (world offsets)
        self.declare_parameter("greedy_offset_xyz", [0.0, -0.6, 0.0])
        self.declare_parameter("window_offset_xyz", [0.0, 0.6, 0.0])
        ...
        self.declare_parameter("pause_topic", "/ik_window/playback/pause")
        self.declare_parameter("seek_topic", "/ik_window/playback/seek")
        self.declare_parameter("progress_topic", "/ik_window/playback/progress")
        self.declare_parameter("time_topic", "/ik_window/playback/time_s")
        self.declare_parameter("cycle_topic", "/ik_window/playback/cycle_s")
        self.declare_parameter("paused_topic", "/ik_window/playback/paused")
        self.declare_parameter("waypoints_topic", "/ik_window/playback/waypoints_json")
        # Republish waypoint timing periodically so late-joining UIs still receive tick marks.
        # Set <= 0 to disable periodic republish.
        self.declare_parameter("waypoints_republish_period_s", 2.0)

        # Marker mode
        # - per_method: separate colored spheres+labels for window/greedy
        # - shared: one white set p0..pN + colored line strips for window/greedy orders + one title per robot
        self.declare_parameter("marker_mode", "per_method")

        # Marker style (used by both modes)
        self.declare_parameter("greedy_rgba", [1.0, 0.95, 0.30, 0.55])  # light yellow
        self.declare_parameter("window_rgba", [0.05, 0.15, 0.80, 0.80])  # deep blue
        ...
        self.window_prefix = self.get_parameter("window_prefix").get_parameter_value().string_value

        self.greedy_offset = list(self.get_parameter("greedy_offset_xyz").get_parameter_value().double_array_value)
        self.window_offset = list(self.get_parameter("window_offset_xyz").get_parameter_value().double_array_value)
        ...
        self.window_rgba = self._rgba_param("window_rgba", (0.05, 0.15, 0.80, 0.80))
        ...
        window_segments, window_order = _build_segments_from_summary(summary, "window")
        ...
        self.window_traj = _segments_to_trajectory(window_segments, joint_names, self.dt, self.hold_time_s)

        self.get_logger().info(
            f"Greedy traj: {len(self.greedy_traj.samples)} samples, total_time={self.greedy_traj.total_time:.3f}s"
        )
        self.get_logger().info(
            f"Window traj: {len(self.window_traj.samples)} samples, total_time={self.window_traj.total_time:.3f}s"
        )

        # Shared UI cycle time (also used for synchronized looping)
        self._cycle_time = float(max(self.greedy_traj.total_time, self.window_traj.total_time))
        ...
        self.window_js_pub = self.create_publisher(JointState, "/window/joint_states", 10)
        self.marker_pub = self.create_publisher(MarkerArray, "/ik_window/markers", 10)
    ...
    def _publish_static_transforms(self) -> None:
        ...
        for prefix, offset in (
            ...
            (self.window_prefix, self.window_offset),
        ):
            ts = TransformStamped()
    ...
    def _offset_for_shared_markers(self) -> Tuple[float, float, float]:
        ...
        We use the greedy offset (and warn if they differ).
        """
        dx = abs(self.greedy_offset[0] - self.window_offset[0])
        dy = abs(self.greedy_offset[1] - self.window_offset[1])
        dz = abs(self.greedy_offset[2] - self.window_offset[2])
        if dx + dy + dz > 1e-6:
            self.get_logger().warn(
                "marker_mode=shared but greedy_offset_xyz != window_offset_xyz; markers will follow greedy offset."
            )
    ...
    # -------------------------
    # Markers: per-method (side-by-side)
    # -------------------------
    def _make_marker_array_per_method(self) -> MarkerArray:
        ...
        wx, wy, wz = float(self.window_offset[0]), float(self.window_offset[1]), float(self.window_offset[2])

        add_text(self.fixed_frame, 1, (gx, gy, gz + 0.9), "GREEDY", self.greedy_rgba, dz=0.0)
        add_text(self.fixed_frame, 2, (wx, wy, wz + 0.9), "WINDOW", self.window_rgba, dz=0.0)
        ...
        add_method_points("window", self.window_prefix, self.window_order, base_id=20000, rgba=self.window_rgba)

        return ma

    # -------------------------
    # Markers: shared (overlay)
    # -------------------------
    def _make_marker_array_shared(self) -> MarkerArray:
        ...
        # Titles (slight X shift to avoid overlap)
        ox, oy, oz = self._offset_for_shared_markers()
        shift = 0.15
        add_text(self.fixed_frame, "ik_window_titles", 1, (ox - shift, oy, oz + 0.9), "WINDOW", self.window_rgba, dz=0.0)
        add_text(self.fixed_frame, "ik_window_titles", 2, (ox + shift, oy, oz + 0.9), "GREEDY", self.greedy_rgba, dz=0.0)

        # Shared path points (white): p1..pN
        targets_sorted = sorted(
            self.targets_by_name.values(),
            key=lambda t: _sort_point_name_key(str(t.get("name", ""))),
        )
        base_id = 100
        label_id = 1100
        ...
        if self._p0_xyz_fixed is not None:
            add_sphere(self.fixed_frame, "ik_window_p0", base_id, self._p0_xyz_fixed, self.shared_point_rgba, scale=0.055)
            add_text(self.fixed_frame, "ik_window_p0_label", label_id, self._p0_xyz_fixed, "p0", self.shared_point_rgba, dz=self.label_dz)
            base_id += 1
            label_id += 1
        ...
        for t in targets_sorted:
            ...
            add_sphere(self.fixed_frame, "ik_window_points", base_id, xyz, self.shared_point_rgba, scale=0.05)
            add_text(self.fixed_frame, "ik_window_point_labels", label_id, xyz, name, self.shared_point_rgba, dz=self.label_dz)
            base_id += 1
            label_id += 1
        ...
        add_line_strip(self.fixed_frame, "ik_window_lines_window", 5000, order_to_xyz(self.window_order), self.window_rgba)
        add_line_strip(self.fixed_frame, "ik_window_lines_greedy", 5001, order_to_xyz(self.greedy_order), self.greedy_rgba)
    ...
    def _publish_waypoints_json(self) -> None:
        ...
        def build(segs: List[Segment], order: List[str]) -> List[dict]:
        ...
        payload = {
            ...
            "window": build(self.window_segments, self.window_order),
            "greedy": build(self.greedy_segments, self.greedy_order),
        }
    ...
    def _on_timer(self) -> None:
        ...
        tw_total = float(self.window_traj.total_time)
        ...
        if self.loop:
            if self.sync_loop:
                ...
                t_w = min(t_cycle, tw_total)
            else:
                # Independent looping.
                t = float(self._t_abs)
                t_g = (t % tg_total) if tg_total > 1e-6 else 0.0
                t_w = (t % tw_total) if tw_total > 1e-6 else 0.0
        else:
            t = float(self._t_abs)
            t_g = min(t, tg_total)
            t_w = min(t, tw_total)
        ...
        if self.window_traj.samples:
            idx_w = int(t_w / self.window_traj.dt)
            self._publish_joint_state(self.window_js_pub, self.window_traj, idx_w)
    ...
    def _try_compute_p0(self) -> None:
        ...
        # Publish start pose once so robot_state_publisher has the correct TF.
        try:
            if self.greedy_traj.samples:
                self._publish_joint_state(self.greedy_js_pub, self.greedy_traj, 0)
            if self.window_traj.samples:
                self._publish_joint_state(self.window_js_pub, self.window_traj, 0)
